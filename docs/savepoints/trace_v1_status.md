...
All V1 goals met. Dielectric BSDF and Russian roulette are deferred to V1 Extensions.
...
| PhotonBeam GL visualization (full AOV suite) | `src/scene_gl.cpp`, `shaders/beam.{vs,fs}` | manual (app) — delivered via AOV task 5 |
...
### V1 Extensions
1. **Dielectric BSDF** — Fresnel reflect/refract; total internal reflection; medium switch on transmission reuses the existing `medium_in/out` plumbing.
2. **Conductor BSDF** — mirror reflection; `Bsdf::sample` returns the perfect specular direction.
3. **Russian roulette** — paths currently terminate only on the hard depth cap; roulette needs per-photon power tracking.

### V2 and Beyond
4. **Photon power / flux** — track power per photon; weight stored points/beams.
